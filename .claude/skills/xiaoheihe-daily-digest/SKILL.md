...
```bash
cd /c/Users/Hveky/Desktop/信息聚合/xiaoheihe-cli
```
...
```bash
cd /c/Users/Hveky/Desktop/信息聚合/xiaoheihe-cli && python xhh.py harvest --top 60 > _today.json
```
...
在调任何 `navigate` / `javascript_tool` / `browser_batch` 之前：

1. `mcp__Claude_in_Chrome__list_connected_browsers`
2. 取 `isLocal: true` 的那台 deviceId
3. `mcp__Claude_in_Chrome__select_browser` 锁定它
4. 如果 `isLocal: true` 不是恰好一台 → 停下让用户选

> 局域网另一台机器（如 SmithHwo）若也登录了同一 Claude 账号，不锁定就可能把 navigate 发到那台 Edge。
