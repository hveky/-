...
Call `mcp__Claude_in_Chrome__list_connected_browsers` to get all connected browsers.

**If only one browser is returned** → use it directly, no further selection needed.

**If multiple browsers are returned** → you MUST explicitly select the local one via `mcp__Claude_in_Chrome__select_browser` before doing anything else. Identify the local browser using this priority order:

1. **label/name contains `Hveky`** (local machine username) → select it
2. **Call `mcp__Claude_in_Chrome__tabs_context_mcp` on each browser** → whichever has a tab on `linux.do` or `linuxdo` is the one the user has open locally → select it
3. **Cannot determine** → stop and ask the user: "检测到多个浏览器连接，请告知应使用哪个：[列表]"

After selecting, call:
```
mcp__Claude_in_Chrome__select_browser  (browserId = chosen ID)
```
All subsequent navigation and JS execution must use this browser.

Then use `mcp__Claude_in_Chrome__tabs_context_mcp` to get the current tab list. The user will typically have the forum already open and logged in — look for the relevant tab.
...
```
file_path: C:\Users\Hveky\Desktop\fourm-research\config.yaml
```

Use `output_dir` from config as the report save path. If the file is missing or `output_dir` is absent, fall back to `C:\Users\Hveky\Desktop\reports\linuxdo`.
...
例如 `C:\Users\Hveky\Desktop\reports\linuxdo\2026-05-24.md`。
